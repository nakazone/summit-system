...
export async function ensureQuoteInvoicesSchema(pool) {
  ...
  try {
    ...
    if (
      (await tableExists(pool, 'quote_invoices')) &&
      (await columnExists(pool, 'quote_invoices', 'invoice_number'))
    ) {
      const [col] = await pool.query(
        `SELECT CHARACTER_MAXIMUM_LENGTH AS len FROM information_schema.COLUMNS
         WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'quote_invoices' AND COLUMN_NAME = 'invoice_number'`
      );
      const len = Number(col[0]?.len) || 0;
      if (len > 0 && len < 48) {
        await pool.query('ALTER TABLE quote_invoices MODIFY invoice_number VARCHAR(48) NOT NULL');
        console.log('[db] quote_invoices.invoice_number alargado para VARCHAR(48).');
      }
    }
