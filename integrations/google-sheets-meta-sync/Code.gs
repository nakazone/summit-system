...
var CONFIG = {
  ...
  API_BASE: 'https://summit-system-production-98eb.up.railway.app',
  /**
...
function ensureCrmSyncedColumn_(sheet) {
  ...
  var want = CONFIG.SYNC_COLUMN_HEADER.toLowerCase();
  for (var i = 0; i < headers.length; i++) {
    if (String(headers[i] || '').trim().toLowerCase() === want) return i;
  }
  var newCol = lastCol + 1;
  sheet.getRange(headerRow, newCol).setValue(CONFIG.SYNC_COLUMN_HEADER);
  return newCol - 1;
}

function syncMetaLeadsToCrmBody_() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();
  ensureCrmSyncedColumn_(sheet);
