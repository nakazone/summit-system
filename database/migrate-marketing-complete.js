...
/** Mesmo caminho que database/load-dotenv-override.mjs (summit-system/.env). */
...
function printIncompleteMysqlEnvHelp() {
  console.error(
    '[migrate] O .env do summit-system está incompleto: não há DATABASE_URL, DATABASE_PUBLIC_URL, MYSQL_URL nem MYSQLHOST,'
  );
  ...
  console.error(
    '     railway ssh -s summit-system -- npm run migrate:marketing-complete'
  );
  console.error(
...
function printRailwayInternalHelp(host) {
  ...
  console.error('  1) No .env de summit-system, acrescente DATABASE_PUBLIC_URL= com a URL "Public network" / TCP (MySQL → Connect).');
  ...
  console.error('     railway ssh -s summit-system -- npm run migrate:marketing-complete');
...
async function main() {
  ...
  if (!cfg) {
    ...
    console.error('  Diagnóstico:', JSON.stringify(getMysqlEnvDiagnostics(), null, 2));
    console.error(
      '  Na Railway (dentro do container): railway ssh -s summit-system -- npm run migrate:marketing-complete'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (e?.code === 'ETIMEDOUT') {
      ...
      if (isRailwayPublicMysqlHostname(cfg.host)) {
        ...
        console.error(
          '  • Melhor opção (migrate no container, rede privada Railway): railway ssh -s summit-system -- npm run migrate:marketing-complete'
        );
        console.error(
    ...
    if (e?.code === 'ECONNREFUSED') {
      ...
      if (isLocalMysqlHost(cfg.host)) {
        ...
        console.error(
          '  • Ou no container: railway ssh -s summit-system -- npm run migrate:marketing-complete'
        );
        console.error('  Diagnóstico:', JSON.stringify(getMysqlEnvDiagnostics(), null, 2));
      } else {
