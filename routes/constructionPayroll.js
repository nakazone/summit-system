...
/** PDF de um recibo (marca Summit Flooring, alinhado ao quote PDF). */
export async function getEmployeePaySlipPdf(req, res) {
  try {
    ...
    const fn = `Summit-Flooring-Pay-Slip-${safePaySlipFilenamePart(row.name)}-${periodId}.pdf`;
...
export async function getIndividualReportsPdf(req, res) {
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      `inline; filename="Summit-Flooring-relatorios-individuais-${periodId}.pdf"`
    );
    res.send(buf);
  } catch (err) {
    if (err.statusCode === 404) {
...
export async function postDistributePaySlips(req, res) {
  try {
    ...
    for (const row of rows) {
      ...
      const buf = await buildPayrollSlipPdfBuffer({ period: p, employeeRow: row });
      const subj = `Summit Flooring — recibo de pagamento (${String(p.name || 'período').slice(0, 42)})`;
      const first = String(row.name || '').trim().split(/\s+/)[0] || '';
      const html = `<p>Olá${first ? ` ${escapeHtmlPaySlip(first)}` : ''},</p><p>Em anexo está o seu recibo de pagamento referente a <strong>${escapeHtmlPaySlip(periodLabel)}</strong>.</p><p>Atenciosamente,<br>Summit Flooring</p>`;
      const r = await sendQuoteEmail({
        to: email,
        subject: subj,
        html,
        pdfBuffer: buf,
        filename: 'Summit-Flooring-Pay-Slip.pdf',
      });
      results.push({
        employee_id: row.employee_id,
        name: row.name,
