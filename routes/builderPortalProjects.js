...
export async function postBuilderProjectPhotos(req, res) {
  return res.status(403).json({
    success: false,
    error:
      'Builders cannot upload site photos. Summit Flooring adds photos from the CRM; use this tab to view them.',
  });
}
...
export async function putBuilderProjectChecklist(req, res) {
  try {
    ...
    if (String(item.assigned_to || 'sf').toLowerCase() !== 'builder') {
      return res.status(403).json({
        success: false,
        error: 'This item can only be completed by Summit Flooring',
      });
    }
  ...
  try {
    ...
    if (!cid) {
      return res.json({
        success: true,
        data: [],
        hint: 'Builder account is not linked to a CRM customer. Contact Summit Flooring.',
      });
    }
