/**
 * Builder ? Summit Flooring messaging.
 */
    ...
    if (!isInternal) {
      ...
      if (senderType === 'admin') {
        logBuilderActivity(pool, {
          ...
          type: 'message_sf',
          text: `Summit Flooring sent a message${onProj}: ${snippet}`,
    ...
    if (!isInternal) {
      ...
      if (senderType === 'admin' && b[0]?.email && builderWantsEmail(b[0].notification_prefs, 'messages')) {
        ...
        sendBuilderNotification({
          to: b[0].email,
          subject: 'New message from Summit Flooring',
        ...
        notifyBuilder(pool, builderId, {
          type: 'message',
          title: 'New message from Summit Flooring',
