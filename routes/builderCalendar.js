...
export async function postBuilderVisitRequest(req, res) {
  try {
    ...
    await notifyBuilder(pool, bid, {
      ...
      body: 'Summit Flooring will confirm your requested date.',
...
export async function exportBuilderCalendarIcs(req, res) {
  try {
    ...
    const lines = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//Summit Flooring//Builder Portal//EN',
      'CALSCALE:GREGORIAN',
    ];
    ...
    try {
      ...
      visits.forEach((v, i) => {
        ...
        lines.push(`UID:sf-visit-${v.id || i}@summitflooring`);
    ...
    reqs.forEach((r, i) => {
      ...
      lines.push('BEGIN:VEVENT');
      lines.push(`UID:sf-req-${r.id || i}@summitflooring`);
      lines.push(`DTSTART:${start}`);
      lines.push(`DTEND:${end}`);
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="summit-flooring-schedule.ics"');
    res.send(lines.join('\r\n'));
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
