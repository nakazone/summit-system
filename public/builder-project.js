(function () {
  ...
  function sfContactBadge() {
    if (window.builderPortalCommon?.sfContactBadgeHtml) {
      return window.builderPortalCommon.sfContactBadgeHtml('Summit Flooring team member');
    }
    return '<span class="bp-sf-contact-badge" title="Summit Flooring"><img src="/assets/logoSummitFlooring.png?v=20260529" alt="Summit Flooring" width="26" height="26" /></span>';
  }
    ...
    const cards = roles
      .map((member) => {
        ...
        const contact = member.name
          ...
          : `<p class="bp-team-card__empty">To be assigned by Summit Flooring</p>`;
      ...
      .join('');
    return `<section class="bp-proj-team" aria-label="Summit Flooring project team">
      <h2 class="bp-proj-team__title">Your Summit Flooring team</h2>
  ...
  function photoMeta(p) {
    const who = p.partner_upload ? 'You' : 'Summit Flooring';
  ...
  function responsibleLabel(assigned) {
    ...
    return a === 'builder' ? 'Builder' : 'Summit Flooring';
  }
  ...
  function renderChecklist(groups, progress) {
    ...
    if (!builder.length && !sf.length && !awaiting.length) {
      return `<div class="bp-card bp-checklist-empty">
        ...
        <p class="bp-muted">Summit Flooring will enable items when your project is scheduled. When available, you will see:</p>
        <ul>
          <li>Your action items (with checkboxes)</li>
          <li>Items awaiting SF approval</li>
          <li>Summit Flooring responsibilities</li>
        </ul>
    ...
    const block = (title, items, canToggle) =>
      items.length
        ...
        : '';
    return `${prog}${block('Your responsibility', builder, true)}${block('Awaiting approval', awaiting, false)}${block('Summit Flooring responsibility', sf, false)}`;
  }
  ...
  async function loadProjectMessages(panel) {
    ...
    messages.forEach((m) => {
      ...
      const sender = mine ? 'You' : 'Summit Flooring';
      const att =
    ...
    if (!messages.length) {
      html += '<p class="bp-muted" style="text-align:center;padding:24px">No messages yet. Contact your Summit Flooring manager below.</p>';
    }
  ...
  function renderMaterials(materials) {
    ...
    if (!rows.length) {
      return '<div class="bp-card"><p class="bp-muted">No materials shared for this project yet. Summit Flooring will publish selections for your approval before work begins.</p></div>';
    }
          ...
          status === 'pending'
            ? `<div class="bp-mat-card__actions">
              ...
              <label class="bp-mat-card__comment-label">Comment (optional)
                <textarea class="bp-mat-comment" data-mid="${m.id}" rows="2" placeholder="Notes for Summit Flooring…"></textarea>
              </label>`
    ...
    if (activeTab === 'summary') {
      ...
      panel.innerHTML = `
        <div class="bp-summary-grid">
          ...
          <div class="bp-card">
            ...
            ${docList}
            <h3 style="margin:16px 0 10px;font-size:14px">Notes from Summit Flooring</h3>
      ...
      panel.innerHTML = `
        <div class="bp-card" style="margin-bottom:16px">
          ...
          <p class="bp-muted" style="margin:0;font-size:13px">Photos added by Summit Flooring for this project. Tap any image to view full size.</p>
        </div>
  ...
  async function load() {
    try {
      ...
      if (!j.data?.project) {
        app.innerHTML = `<p class="bp-card">Project data is incomplete. Please try again or contact Summit Flooring.</p>`;
        return;
      }
      state = j.data;
  ...
  function openEvaluationModal(googleReviewUrl) {
    ...
    modal.innerHTML = `<div class="bp-modal__box">
      <h3>How was this project with Summit Flooring?</h3>
    ...
    modal.querySelector('#evalSubmit')?.addEventListener('click', async () => {
      ...
      if (reviewUrl && confirm('Thank you! Would you like to leave a Google review for Summit Flooring?')) {
