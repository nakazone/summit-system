/**
 * Lista de projetos — Summit Flooring CRM
 */
let state = {
  projects: [],
  filtered: [],
...
function showSkeletons() {
  ...
  el.innerHTML = Array.from({ length: 6 })
    .map(
      () =>
        `<div class="sf-card" style="min-height:180px;animation:pulse 1.2s ease-in-out infinite;background:rgba(28, 28, 31,.06)"></div>`
    )
    .join('');
}
    ...
    <div class="sf-card project-card" role="link" tabindex="0" data-href="/project-detail.html?id=${p.id}">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px">
        <div style="display:flex;gap:6px;flex-wrap:wrap">
          ...
          ${p.flooring_type ? `<span class="sf-stage" style="background:rgba(28, 28, 31,.08);color:var(--sf-navy)">${p.flooring_type}</span>` : ''}
      ...
      <div style="height:3px;background:rgba(28, 28, 31,.1);border-radius:3px;margin-bottom:10px">
