...
(function () {
  ...
  function renderSimulationDetail(sim, result) {
    ...
    host.innerHTML = `<div class="bp-card bp-pricing-sim-detail">
      ...
      <p class="bp-muted" style="font-size:12px;margin-top:10px">Illustrative only. Final quotes depend on site conditions, materials, and scope confirmed by Summit Flooring.</p>
  ...
  async function loadPortal() {
    ...
    const validLine = portalMeta.valid_through
      ...
      : 'Partner rates - contact Summit Flooring for an updated table.';
    ...
    const hdr =
      window.builderPortalCommon?.pageHeaderHtml?.({
        ...
        subtitle: 'Read-only partner rate sheet. Contact Summit Flooring to request changes.',
