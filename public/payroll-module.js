...
async function downloadPaySlipsZip(format) {
  try {
    ...
    downloadBlob(blob, `Summit-Flooring-recibos-${ext}-${selectedPeriodId}.zip`);
...
async function sendPaySlipsEmail() {
  ...
  if (
    !confirm(
      'Enviar um e-mail por funcionário com o recibo em PDF (marca Summit Flooring)? Inclui apenas quem tem e-mail no cadastro e dados neste período. É necessário Resend ou SMTP configurado no servidor.'
    )
  ) {
    return;
  }
...
function buildTotalReportFragment(d, from, to) {
  ...
  return `<div class="rounded-xl border-2 border-[#1c1c1f]/20 bg-[#1c1c1f] text-white overflow-hidden shadow-sm">
    <h3 class="font-bold px-4 py-3 text-[#d6c4a8] text-xs uppercase tracking-wide">Resumo financeiro</h3>
    <div class="px-4 pb-4">
